...
function optionSelected(answer){
    ...
    if(userAns == correcAns){ //if user selected option is equal to array's correct answer
        ...
        +'<p class="subtitle_esatto">Sì, questa frase è presa dal programma <b>'+ nome_lista + '</b></p>'
        ...
        function setGlobal(){
          ...
          if (element.idPartito===obj[que_count].risposta_idPartito) {
            ...
            if (element.risposta_valore==="grey" || element.risposta_valore=== null ) {
              ...
              risposta_text1='<img src="img/Sbagliato_X.svg" class="icon_correct" alt="">'
              + '<h3 class="titolo_sbagliato">Risposta sbagliata</h3>'
              +'<p class="subtitle_esatto">La tua scelta,<b>'+ nome_lista_sbagliata +'</b> si esprime così nel programma: </p> <br>' 
        ...
        let risposta_text= '<img src="img/iconcorrect.svg.png" class="icon_correct" alt="">'
        +'<h3 class="titolo_esatto">Risposta esatta</h3>'
        +'<p class="subtitle_esatto">La risposta corretta è: <b>'+ nome_lista + '</b></p>'
        risposta_corretta.innerHTML=risposta_text
        risposta_corretta.style.opacity=1;
        ...
        const opacità2 = document.querySelectorAll('.dot')
        for(i=0; i < opacità2.length; i++){
        opacità2[i].style.opacity=0.5}
        const opacità = document.querySelectorAll('.dot.correct')
        for(i=0; i < opacità.length; i++){
        opacità[i].style.opacity=1}
        const opacità1 = document.querySelectorAll('.dot.incorrect')
        for(i=0; i < opacità1.length; i++){
            opacità1[i].style.opacity=1}
...
function showResult(){
  ...
  for(i=0; i < numberall2; i++){
    ...
    if (userScore > 15){ // if user scored more than 3
      ...
      let scoreTag= '<h1 class="score_title">Si può fare meglio!</h1>'
